...
def save_game_session_data(request):
	if request.method == 'POST':
		game_session_data = request.POST.get('game_session_data')

		# Save the game session data in a JSON file or database
		# For example, save it in a JSON file
		with open('static/game_session_data.json', 'a') as file:
